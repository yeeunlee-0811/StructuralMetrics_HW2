...
for coeff = 1:3
    ...
    epspb0 = (ones(n,1)-y).*max(1-cdf('Normal',Xfit0,0,1),0.00001)+y.*max(cdf('Normal',Xfit0,0,1),0.00001);
    llobji0 = log(epspb0);
    
    Xfit = X*param;
    epspb = (ones(n,1)-y).*max(1-cdf('Normal',Xfit,0,1),0.00001)+y.* max(cdf('Normal',Xfit,0,1),0.00001);
    llobji = log(epspb);
